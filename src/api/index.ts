...
import {
  ...
  type T_SWAPI_VEHICLE,
  type T_SWAPI_SPECIES_REDUCED,
  SCHEMA_SWAPI_SPECIES_REDUCED,
  type T_SWAPI_VEHICLE_REDUCED,
  SCHEMA_SWAPI_VEHICLE_REDUCED,
  type T_SWAPI_PEOPLE,
  type T_SWAPI_PEOPLE_REDUCED,
  SCHEMA_SWAPI_PEOPLE_REDUCED,
  SCHEMA_SWAPI_SEARCH_RESPONSE,
  SCHEMA_SWAPI_PEOPLE,
  type T_SWAPI_PEOPLE_SEARCH,
  SCHEMA_SWAPI_PEOPLE_SEARCH,
  SCHEMA_SWAPI_VEHICLE,
  SCHEMA_SWAPI_SPECIES,
  type T_SWAPI_SPECIES,
} from './schemas.ts'
...
const chara_api = wretch('https://swapi-api.hbtn.io/api', {
  mode: 'cors',
  headers: {
    'Content-Type': 'application/json',
  },
...
export async function get_all_pages<T>(path: string, schema: z.ZodMiniType) {
  ...
  while (next) {
    const { success: fetching_success, error: fetching_error } = await unwrap<unknown>(
      api.get(next),
    )
...
export async function get_all_vehicles(): Promise<T_SWAPI_VEHICLE_REDUCED[]> {
  const all_vehicles = await get_all_pages<T_SWAPI_VEHICLE_REDUCED>(
    '/vehicles',
    SCHEMA_SWAPI_VEHICLE_REDUCED,
  )
  return all_vehicles
...
export async function get_all_species(): Promise<T_SWAPI_SPECIES_REDUCED[]> {
  const all_species = await get_all_pages<T_SWAPI_SPECIES_REDUCED>(
    '/species',
    SCHEMA_SWAPI_SPECIES_REDUCED,
  )
  return all_species
...
export async function get_all_people(): Promise<T_SWAPI_PEOPLE_REDUCED[]> {
  const all_people = await get_all_pages<T_SWAPI_PEOPLE_REDUCED>(
    '/people',
    SCHEMA_SWAPI_PEOPLE_REDUCED,
  )
  return all_people
...
export async function search_people(params: { name: string }): Promise<T_SWAPI_PEOPLE_SEARCH[]> {
  ...
  const { success: fetching_success, error: fetching_error } = await unwrap(
    api.query({ name: params.name }).get('/people/'),
  )
  ...
  const { data: people_list, error: parsing_error } = SCHEMA_SWAPI_SEARCH_RESPONSE(
    SCHEMA_SWAPI_PEOPLE_SEARCH,
  ).safeParse(fetching_success)
...
export async function get_vehicle(params: { uid: string }): Promise<T_SWAPI_VEHICLE> {
  console.log('vehicle', params.uid)

  const { success: fetching_success, error: fetching_error } = await unwrap(
    api.get(`/vehicles/${params.uid}`),
  )
...
export async function get_species(params: { uid: string }): Promise<T_SWAPI_SPECIES> {
  console.log('species', params.uid)

  const { success: fetching_success, error: fetching_error } = await unwrap(
    api.get(`/species/${params?.uid}`),
  )
...
export async function get_people(params: { uid: string }) {
  const { success: fetching_success, error: fetching_error } = await unwrap(
    chara_api.get(`/people/${params.uid}`),
  )
